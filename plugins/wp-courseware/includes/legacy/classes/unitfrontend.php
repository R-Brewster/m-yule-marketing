	...
	class WPCW_UnitFrontend {
		...
		public function render_quizzes_showAllCorrectAnswers( $returnDataInArray = false ) {
			...
			if ( $this->unitQuizDetails->questions && count( $this->unitQuizDetails->questions ) > 0 ) {
				...
				foreach ( $this->unitQuizDetails->questions as $question ) {
					...
					if ( $setting_showCorrectAnswer && in_array( $question->question_type, array(
							'truefalse',
							'multi'
						) ) ) {
		...
		public function check_quizzes_workoutQuizPassStatusDetails() {
			...
			if ( $this->check_quizzes_validQuizDetails() &&
				...
				/**
				 * Action: Quiz before pass status details.
				 *
				 * @since 4.6.3
				 *
				 * @param object The Unit Quiz progress.
				 * @param object The Unit Quiz details.
				 */
				do_action( 'wpcw_quiz_before_pass_status_details', $this->unitQuizProgress, $this->unitQuizDetails );
				...
				if ( $this->unitQuizProgress->quiz_needs_marking == 0 ) {
					...
					if ( $this->unitQuizDetails->has_open_questions ) {
						...
						$rtnDetails[0]['msg_overall_grade'] = sprintf(
							__( 'You got %1$d out of %2$d questions <strong>(%3$s%%)</strong> correct!', 'wp-courseware' ),
					...
					/**
					 * Filter: Message Overall Quiz Grade.
					 *
					 * @since 4.6.3
					 *
					 * @param string The quiz overall grade message.
					 * @param object The unit quiz progress object.
					 * @param object The unit quiz details object.
					 *
					 * @return string The quiz overall grade message.
					 */
					$rtnDetails[0]['msg_overall_grade'] = apply_filters( 'wpcw_quiz_overall_grade_message', $rtnDetails[0]['msg_overall_grade'], $this->unitQuizProgress, $this->unitQuizDetails );
				...
				/**
				 * Action: Quiz after pass status details.
				 *
				 * @since 4.6.3
				 *
				 * @param object The Unit Quiz progress.
				 * @param object The Unit Quiz details.
				 */
				do_action( 'wpcw_quiz_after_pass_status_details', $this->unitQuizProgress, $this->unitQuizDetails );
			} // end complex if


			/**
			 * Filter: Quiz Pass Status Details.
			 *
			 * @since 4.6.3
			 *
			 * @param array The pass status details.
			 * @param object The quiz progress object.
			 * @param object The quiz details object.
			 *
			 * @return array The pass status details.
			 */
			return apply_filters( 'wpcw_quiz_pass_status_details', $rtnDetails, $this->unitQuizProgress, $this->unitQuizDetails );
		}
		...
		", $this->currentUserID, $this->unitPost->ID, $this->unitQuizDetails->quiz_id );
			...
			if ( $weNeedToUpdateTheMostRecentProgress ) {
				$SQL = arrayToSQLUpdate( $wpcwdb->user_progress_quiz, $data, array(
					'user_id',
					'unit_id',
					'quiz_id',
					'quiz_attempt_id'
		...
		public function check_quizzes_validQuizDetails() {
			if ( apply_filters( 'wpcw_unit_quiz_allow_quiz_progress_without_questions', false ) ) {
				return ( ( is_array( $this->unitQuizDetails ) || is_object( $this->unitQuizDetails ) ) && count( (array) $this->unitQuizDetails ) > 0 );
			}
		...
		public function fetch_paging_getQuestion_moveQuestionMarker( $jumpMode ) {
			...
			if ( ! empty( $this->unitQuizProgress ) ) {
				...
				if ( isset( $questionID ) && ! $this->unitQuizProgress->quiz_data[ $questionID ]['possible_answers'] ) {
					...
					$quizQuestion     = str_replace( array(
						'[',
						']'
					), '', WPCW_arrays_getValue( $_POST, 'quiz_question' ) );
					$quizAnswersName  = str_replace( array(
						'[',
						']'
					), '', WPCW_arrays_getValue( $_POST, 'quiz_answers_name' ) );
